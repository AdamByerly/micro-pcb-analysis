...
class Loops(Loggable):
    ...
    def _train(self, epoch):
        dataset = self._strategy.experimental_distribute_datasets_from_function(
            lambda input_context: self._dataset_distribute(input_context,
                self._in_pipe.get_training_dataset()))
        ...
        for features, labels in dataset:
            ...
            if not self._graph_saved:
                self._out.log_graph_of_func(
                    self._distributed_train_step, features, labels)
                self._graph_saved = True

            loss = self._distributed_train_step(features, labels)

            self._ema_weights.update(self._in_pipe.get_train_steps())
            self._out.train_step_end(epoch, step, loss, lr,
                self._in_pipe.get_train_steps())

            step += 1

        self._out.train_end(self._model,
            self._optimizer, self._ema_weights, epoch)
    ...
    def _validate(self, epoch):
        ...
        dataset = self._strategy.experimental_distribute_datasets_from_function(
            lambda input_context: self._dataset_distribute(input_context,
                self._in_pipe.get_validation_dataset()))

        step = 0
        for features, labels in dataset:
            loss, top1, top5 = self._distributed_eval_step(features, labels)

            self._metrics.update_loss(loss)
            self._metrics.update_accuracy(top1, top5)

            self._out.validation_step_end(step,
                self._in_pipe.get_validation_steps())
    ...
    def _distributed_train_step(self, features, labels):
        def train_step(features_, labels_):
            with tf.GradientTape() as tape:
                logits = self._model.forward(features_, True)
                with tf.name_scope("loss"):
                    loss = tf.keras.losses.categorical_crossentropy(
                            labels_, logits, from_logits=True)
                    loss = tf.nn.compute_average_loss(loss, self._batch_size)
        ...
        losses = self._strategy.run(
            train_step, args=(features, labels))
    ...
    def _distributed_eval_step(self, features, labels):
        def validation_step(features_, labels_):
            with tf.name_scope("validation"):
                logits = self._model.forward(features_, False)
                loss   = tf.keras.losses.categorical_crossentropy(
                            labels_, logits, from_logits=True)
                loss   = tf.nn.compute_average_loss(loss, self._batch_size)
                top1   = tf.reduce_mean(
        ...
        losses, top1s, top5s = self._strategy.run(
            validation_step, args=(features, labels))
