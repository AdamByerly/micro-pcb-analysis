...
def fc(op_name, is_training, in_tensor, variable):
    with tf.name_scope(op_name):
...
def max_pool(op_name, in_tensor,
    ...
    with tf.name_scope(op_name):
        return tf.nn.pool(input=in_tensor, window_shape=window_shape,
            strides=strides, pooling_type="MAX", padding=padding, name=op_name)


def conv_3x3(op_name, is_training, in_tensor, variable, strides=None):
    if strides is None:
        strides = [1, 1, 1, 1]
    with tf.name_scope(op_name):
        x = tf.nn.conv2d(in_tensor, variable.variable,
            strides=strides, padding="VALID", name=op_name)
        x = batch_norm("bn_" + op_name, is_training, x, variable)
        return tf.nn.relu(x, "relu_" + op_name)
...
def hvc(op_name, is_training, in_tensor, variable):
    with tf.name_scope(op_name):
